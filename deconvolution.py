...
decon_runs = "input/GAr/GAr_SiPM1_DAY21_OV2_T-180.txt"
...
if deconvolve == "SPE": #USE SPE SIGNAL TO DECONVOLVE
    las = import_scint_prof(paths[0]+paths[3],timebin,normalize=norm,trim=trm,align=shift,start=s_start,cut_i=init,cut_f=fnal,invert=False)
    if len(las.wvf) > len(alp.wvf):
        las = import_scint_prof(paths[0]+paths[3],timebin,normalize=norm,trim=trm,align=shift,start=s_start,cut_i=init,cut_f=fnal+len(las.wvf)-len(alp.wvf),invert=False)
...
spe = import_scint_prof(paths[0]+paths[2],timebin,normalize=norm,trim=trm,align=shift,start=s_start,cut_i=init,cut_f=fnal,invert=False)
...
plt.title("Deconvolution result for %s"%filename)
...
f = ROOT.TFile("output/"+filename+"_f%i.root"%filter_strenght, "RECREATE")
